...
export const GumletPlayer = forwardRef(({ 
  ...
  useEffect(() => {
    ...
    // Add event listeners once player is ready
    player.on('ready', () => {
    ...
    player.on('playbackRateChange', (e) => {
      if (props.onPlaybackRateChange) props.onPlaybackRateChange(e);
    });
  }, [iframeRef, props]);
  ...
  const getPaused = () => {
    return new Promise((resolve) => {
      if (playerJSObject) playerJSObject.getPaused((e) => resolve(e));
    });
  };

  const getMuted = () => {
    return new Promise((resolve) => {
      if (playerJSObject) playerJSObject.getMuted((e) => resolve(e));
    });
  };

  const getVolume = () => {
    return new Promise((resolve) => {
      if (playerJSObject) playerJSObject.getVolume((e) => resolve(e));
    });
  };

  const getDuration = () => {
    return new Promise((resolve) => {
      if (playerJSObject) playerJSObject.getDuration((e) => resolve(e));
    });
  };

  const getCurrentTime = () => {
    return new Promise((resolve) => {
      if (playerJSObject) playerJSObject.getCurrentTime((e) => resolve(e));
    });
  };

  const getPlaybackRate = () => {
    return new Promise((resolve) => {
      if (playerJSObject) playerJSObject.getPlaybackRate((e) => resolve(e));
    });
  };

  let version = props.version ? `-${props.version}` : '';
  ...
  return (
    <div style={style}>
      {JSON.stringify(schemaOrgVideoObject) !== '{}' && (
        <script type="application/ld+json" dangerouslySetInnerHTML={{ __html: JSON.stringify(schemaOrgVideoObject) }} />
      )}
